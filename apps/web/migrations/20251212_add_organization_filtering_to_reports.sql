-- Migration: Add organization-based filtering to reports_dashboard_metrics
-- Description: Implements organization context filtering in reports:
--   - Add p_organization_id parameter to RPC function
--   - Validate organization_id exists in organizations table
--   - Filter all queries by organization_id from shifts, facilities, medical_staff
--   - Ensure RLS policies are applied to prevent cross-organization data access
...
DECLARE
    ...
    v_trunc_unit TEXT;
BEGIN
    -- Validate organization_id exists when provided
    -- This ensures data integrity and prevents queries against non-existent organizations
    IF p_organization_id IS NOT NULL THEN
        ...
        IF NOT v_organization_exists THEN
            RAISE EXCEPTION 'Organization with id % does not exist', p_organization_id
                USING HINT = 'Please provide a valid organization_id from the organizations table';
        END IF;
    ELSE
        RAISE EXCEPTION 'organization_id parameter is required'
            USING HINT = 'Please provide a valid organization_id to filter reports';
    END IF;
    ...
    IF p_unit != 'todas' THEN
        ...
        IF NOT v_facility_exists THEN
            RAISE EXCEPTION 'Facility with id % does not exist in organization %', p_unit, p_organization_id
    ...
    -- ORGANIZATION FILTER: Filter by s.organization_id = p_organization_id
    SELECT
        COUNT(*),
    ...
    -- ORGANIZATION FILTER: Filter by pr.organization_id = p_organization_id
    ...
    -- Build specialty trend (time series with dynamic bucketing)
    -- ORGANIZATION FILTER: Filter by s.organization_id = p_organization_id
    WITH date_series AS (
        SELECT
            generate_series(
    ...
    -- Build efficiency trend (occupancy and SLA over time from shift_attendance)
    -- ORGANIZATION FILTER: Filter by s.organization_id = p_organization_id
    WITH date_series AS (
        SELECT
            generate_series(
    ...
    period_stats AS (
        SELECT
            ...
            AVG(
                CASE
                    WHEN sa.check_in_at IS NOT NULL AND s.start_time IS NOT NULL
                    THEN EXTRACT(EPOCH FROM (sa.check_in_at - s.start_time)) / 60
                    ELSE NULL
                END
            ) as avg_sla_minutes
        FROM date_series ds
    ...
    SELECT json_agg(
        json_build_object(
            ...
            'sla', ROUND(COALESCE(avg_sla_minutes, 0), 1)
        ) ORDER BY period_start
    )
    ...
    -- Build highlights (top performers by shift count with period-over-period variation)
    -- This query joins shifts with medical_staff and facilities to get top performers
    -- Calculates real variation by comparing current period vs previous period shift counts
    -- ORGANIZATION FILTER: Filter by s.organization_id = p_organization_id
    WITH current_period_stats AS (
        ...
        FROM shifts s
        INNER JOIN medical_staff ms ON s.staff_id = ms.id
        LEFT JOIN facilities f ON s.facility_id = f.id
        WHERE s.start_time >= v_start_date
            AND s.start_time <= v_end_date
            AND s.status = 'accepted'
    ...
    top_performers AS (
        SELECT
            curr.staff_name,
            curr.staff_specialty,
            curr.facility_name,
            curr.shift_count,
            ROUND(
                CASE
                    WHEN prev.shift_count > 0 THEN
                        ((curr.shift_count::NUMERIC - prev.shift_count::NUMERIC) / prev.shift_count::NUMERIC * 100)
                    WHEN curr.shift_count > 0 THEN 100
                    ELSE 0
                END,
                1
            ) as variation
        FROM current_period_stats curr
        LEFT JOIN previous_period_stats prev ON curr.staff_id = prev.staff_id
        ORDER BY curr.shift_count DESC
        LIMIT 5
    )
    SELECT json_agg(
        json_build_object(
...
GRANT EXECUTE ON FUNCTION reports_dashboard_metrics(TEXT, TEXT, TEXT, UUID) TO authenticated;
...
- Highlights: Top performers with period-over-period variation from real shift counts
- All metrics include period-over-period delta percentages
Filters by organization_id (REQUIRED), specialty and facility (unit) parameters.
Organization-based RLS ensures users only see data from their organization.';
