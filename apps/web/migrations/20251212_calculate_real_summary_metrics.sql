-- Migration: Calculate real summary metrics from database
-- Description: Implements actual metric calculations for dashboard:
--   - Total attendances from shifts count
--   - Occupancy rate from shift_attendance check-in/check-out data
--   - Revenue from payment_records.total_amount sum
--   - Denial/glosa rate from payment_records metadata
--   - Period-over-period deltas for all metrics
