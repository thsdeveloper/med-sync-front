-- Migration: Create reports_dashboard_metrics RPC function
-- Description: Aggregates real data from shifts, medical_staff, facilities, and payment_records
-- Returns: JSON structure matching ReportMetricsBundle TypeScript interface
...
BEGIN
    ...
    WHERE s.start_time >= v_start_date
        ...
        AND (p_specialty = 'todas' OR LOWER(ms.specialty) = LOWER(p_specialty))
    ...
    specialty_counts AS (
        ...
        LEFT JOIN medical_staff ms ON s.staff_id = ms.id
        LEFT JOIN facilities f ON s.facility_id = f.id
        WHERE (p_unit = 'todas' OR f.id::TEXT = p_unit)
...
COMMENT ON FUNCTION reports_dashboard_metrics IS 'Aggregates dashboard metrics from shifts, medical_staff, facilities, and payment_records tables. Returns JSON matching ReportMetricsBundle TypeScript interface.';
