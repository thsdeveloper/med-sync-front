...
BEGIN
    ...
    -- FIXED: Now respects p_specialty parameter
    WITH date_series AS (
        SELECT
            generate_series(
...
COMMENT ON FUNCTION reports_dashboard_metrics IS 'Aggregates dashboard metrics from shifts, medical_staff, facilities, and payment_records tables. Returns JSON matching ReportMetricsBundle TypeScript interface. Filters by specialty and unit parameters, handling NULL specialty values with COALESCE.';
